...
/obj/effect/rune/convert/proc/do_convert(mob/living/convertee, list/invokers)
	...
	if(convertee.anti_magic_check(TRUE, TRUE))
		...
		return 0
	var/bruteheal = convertee.getBruteLoss() * 0.75
	var/burnheal = convertee.getFireLoss() * 0.75
	if(bruteheal || burnheal)
		convertee.heal_overall_damage(brute = bruteheal, burn = burnheal, organic_only = TRUE)
	convertee.visible_message("<span class='warning'>[convertee] writhes in pain \
	[bruteheal || burnheal ? "even as [convertee.p_their()] wounds heal and close" : "as the markings below [convertee.p_them()] glow a bloody red"]!</span>", \
