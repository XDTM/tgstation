...
/obj/item/bodypart/proc/receive_damage(brute = 0, burn = 0, stamina = 0, only_robotic = FALSE, only_organic = FALSE, updating_health = TRUE)
	if(owner && (owner.status_flags & GODMODE))
		return FALSE	//godmode
		
	if(only_robotic && status != BODYPART_ROBOTIC)
		return FALSE

	if(only_organic && status != BODYPART_ORGANIC)
		return FALSE
