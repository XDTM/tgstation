...
/mob/living/proc/bloodcrawl_consume(mob/living/victim)
	...
	if(victim.reagents && victim.reagents.has_reagent("devilskiss"))
		...
		take_overall_damage(brute = 25) //I can't use adjustHealth() here because bloodcrawl affects /mob/living and adjustHealth() only affects simple mobs
	...
	victim.take_overall_damage(brute = 1000)
	victim.death()
